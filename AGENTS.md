...
CMCENDev uses Semantic Versioning, Conventional Commits, git-cliff, automated Git
...
The `Tag release` Forgejo Actions workflow automatically detects a squash-merge
commit whose subject exactly matches:

```text
chore(release): prepare vX.Y.Z
```

and creates the corresponding annotated release tag on that exact commit.

For example:

```text
chore(release): prepare v0.1.0-beta.5
        |
        +-- tag: v0.1.0-beta.5
```

The tag MUST point to the release-preparation commit on `main`. Do not tag an
earlier feature commit, individual PR commit, release-branch commit, or
arbitrary development commit.

For the normal release flow, maintainers and agents MUST NOT manually create or
push the release tag. Merging the correctly named release-preparation PR into
`main` is the trigger for automatic tagging.

The tagging workflow MUST NOT move, replace, or overwrite an existing release
tag. If the requested tag already exists, automation MUST stop rather than
rewriting release history.

Tags created by release automation use the release automation Git identity and
are not equivalent to a tag personally signed by a maintainer. If a maintainer
explicitly requires a personally signed tag for a special release, that is an
exception to the normal automated flow and requires explicit user direction.

If automatic tagging fails, diagnose and correct the automation through the
normal branch and pull request workflow. Do not bypass the failure by manually
tagging the release unless the user explicitly instructs otherwise.
...
squash merge release PR into main
        |
        v
Tag release workflow detects release-preparation commit
        |
        v
annotated vX.Y.Z tag created and pushed automatically
        |
        v
Publish release workflow runs
        ...
        |                      |
        v                      v
Forgejo Release created   Docker image built and pushed
                               |
                               v
                         Forgejo Packages
```
...
* manually create or push a release tag;

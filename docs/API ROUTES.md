...
| `POST`   | `/api/admin/users`                          | Authenticated + `canProvisionUsers`                                           | Provision an invited account and email a seven-day activation link. Body: `firstName`, `lastName`, `email`, and a built-in non-developer `role`. `internal_beta` requires developer access. Custom roles and content areas are not assigned during invitation.                   |

...
| `POST`   | `/api/profile/encryption`                               | Authenticated                                                    | Explicitly opt in to OpenBao-backed profile encryption. Requires `{ "confirm": true }`; encrypts profile fields and any existing TOTP seed in MongoDB under the account's dedicated Transit key and returns 503 unless Transit is configured. Ghost/dummy accounts are ineligible. Username and email remain clear for sign-in and delivery workflows. |
| `GET`    | `/api/media/:uuid/:variant`                              | Public                                                           | Stable CMCEN media URL for uploaded images. For encrypted uploads, decrypts the private object in memory before returning the image; the MinIO object URL is never exposed. |

...
Ownership is enforced with `createdBy` (or comment `author`) for every role, including administrators. Hidden records are excluded; another user's, hidden, invalid, or missing detail returns `404`. Responses use `Cache-Control: no-store` and explicit field lists, excluding staff identities, internal notes, consent records, contact details, RSVP attendees, and revision history.

...
CMCEN crest, every Jimmy generic-image variant (including `jimmy.jpeg`), and
TD Insurance placeholder images are never uploaded. They reuse
`https://cdn.corebot.ca/cmcen-demo/images/branch-crest/large.webp` instead.
Records with no usable image use that same branch crest fallback; unit-specific
crests and normal photos continue through the media import.

...
// Shared page-builder controller. Each host owns its session and surrounding shell.
(() => {
  function mount({
    api,
    permissions,
    ...
    const pagesAdminStatus = root.querySelector("[data-pages-status]");
    const pagesAdminPage = root.querySelector("[data-pages-page]");
    const pagesAdminContent = root.querySelector(".pages-admin-shell");
    ...
    const renderCleanups = new Set();

    function hasUnsavedChanges() {
      return busy || Boolean(savePromise) || editVersion !== savedVersion;
    }

    async function runAction(action) {
      if (disposed || busy) return;
      busy = true;
      pagesAdminContent.inert = true;
      pagesAdminContent.setAttribute("aria-busy", "true");
      try {
        return await action();
      } catch (error) {
        showPagesActionToast(
          error.message || "Could not complete this action",
          "error",
        );
      } finally {
        busy = false;
        if (!disposed) {
          pagesAdminContent.inert = false;
          pagesAdminContent.setAttribute("aria-busy", "false");
        }
      }
    }
    ...
    async function pageApi(path, options = {}) {
      ...
      try {
        ...
        if (
          !disposed &&
          error.status === 403 &&
          path.startsWith("/api/admin/pages") &&
    ...
    function getNavigationGroupLabel(group) {
      ...
      return (
        localized(group?.label) || CMCENUtils.formatTitleCaseValue(group?.key)
      );
    }
    ...
    async function refreshSiteNavigation() {
      ...
      await Promise.all(
        [...new Set(navigationWindows)].map(async (navigationWindow) => {
          try {
            if (typeof navigationWindow.reloadSiteNavigation !== "function")
              return;
    ...
    function updateCarouselItemLocalized(
      index,
      itemIndex,
      field,
      language,
    ...
    function createPageList() {
      ...
      create.addEventListener("click", () => runAction(createPage));

      if (pagesState.canManageNavigation) actions.append(manageNavigation);
      ...
      if (!pagesState.pages.length) {
        ...
        pagesState.pages.forEach((page) => {
          ...
          button.addEventListener("click", () =>
            runAction(() => loadPageDetail(page._id)),
          );
          list.append(button);
        });
      }
    ...
    function createBlockToolbar(index) {
      ...
      const remove = createBuilderIconButton(
        "Remove block",
        "trash",
        "is-danger",
      );
    ...
    function showCanvasDropPreview(canvas, layout, { valid = true } = {}) {
      ...
      preview.style.setProperty(
        "--page-builder-preview-row",
        String(layout.row),
      );
      preview.style.setProperty(
        "--page-builder-preview-span",
        String(layout.span),
      );
      preview.style.setProperty(
    ...
    function createBlockResizeHandle(index) {
      ...
      const resizeBy = (difference) => {
        ...
        const span = Math.min(
          Math.max(
            getBuilderBlockSpan(block) + difference,
            minBuilderBlockSpan,
          ),
          12,
        );
    ...
    function createBlockHeightResizeHandle(index) {
      ...
      handle.addEventListener("pointerdown", (event) => {
        ...
        const rowStep =
          rowHeight + (Number.parseFloat(canvasStyles.rowGap) || 0);
        handle.setPointerCapture?.(event.pointerId);
        tile.classList.add("is-resizing");
    ...
    function createMediaDropZone({
      ...
      input.addEventListener("change", () => {
        ...
        if (file) runAction(() => onUpload(file));
      });
      ...
      if (progress) {
        ...
        if (progress.status === "uploading") {
          progressWrapper.append(
            CMCENUtils.createLoadingSpinner("Uploading image..."),
          );
        } else {
    ...
    function createCarouselItemEditor(block, index, item, itemIndex) {
      ...
      remove.addEventListener("click", () =>
        removeCarouselItem(index, itemIndex),
      );
    ...
    function createBlockEditor(block, index) {
      ...
      renderCleanups.add(stopPointerDrag);
    ...
    function createPageAccessEditor() {
      ...
      audienceLabel.className =
        "admin-editor-field admin-editor-field--compact";
    ...
    function createPageEditor() {
      ...
      backToPages.addEventListener("click", () =>
        runAction(async () => {
          if (
            (editVersion !== savedVersion || savePromise) &&
            !(await saveSelectedPage({ toast: false }))
          )
            return;
          returnToPageLibrary();
        }),
      ...
      openPage.addEventListener("click", () => {
        ...
        window.open(
          pagesState.selectedPage.route ||
            `/pages/${pagesState.selectedPage.slug}`,
          "_blank",
          "noopener",
        );
      });

      pageActions.append(backToPages);
      if (pagesState.canManageNavigation) pageActions.append(manageNavigation);
      pageActions.append(openPage);
      ...
      continueToBuilder.addEventListener("click", () =>
        runAction(() => saveSelectedPage({ auto: false })),
      );
      ...
      continueToNavigation.textContent = hasPublishedNavigationLink
        ...
        : "Save and continue";
      continueToNavigation.addEventListener("click", () =>
        runAction(async () => {
          ...
          if (saved) {
            setPagesState({
              ...
              navigationMode: "add",
              navigationStep: pagesState.canManageNavigation
                ? "setup"
                : "publish",
            });
          }
      ...
      saveDraft.addEventListener("click", () =>
        runAction(saveDraftAndReturnToLibrary),
      );
      ...
      remove.addEventListener("click", () => runAction(deleteSelectedPage));
    ...
    function createPagePreviewModal() {
      ...
      pagePreviewPositionCleanup = positionPagePreviewInViewport(
        overlay,
        preview,
      );
      return overlay;
    ...
    function createNavigationPanel() {
      ...
      const renderGroupOptions = () => {
        ...
        matches.forEach((group) => {
          ...
          option.setAttribute(
            "aria-selected",
            String(group.key === selectedGroup),
          );
        ...
        if (groupItem) {
          ...
          removeGroup.addEventListener("click", async () => {
            ...
            if (
              ...
              return;
            await runAction(() => deleteNavigationItem(groupItem._id));
          });
        ...
        links.forEach((item) => {
          ...
          remove.addEventListener("click", async () => {
            if (
              ...
              return;
            await runAction(() => deleteNavigationItem(item._id));
          });
    ...
    async function continueNavigationSetup({
      ...
      try {
        if (group === "__new__") {
          ...
          if (!headerName) {
            showPagesActionToast(
              "Name the new header before continuing",
              "error",
            );
            return;
          }
    ...
    function createNavigationModal() {
      ...
      close.addEventListener("click", closeNavigationModal);
    ...
    function createMediaPickerModal() {
      ...
      refresh.textContent = pagesState.mediaIsLoading
        ? "Loading..."
        : "Refresh";
    ...
    async function saveSelectedPage({ auto = false, toast = true } = {}) {
      // Serialize writes: a slower autosave must never overwrite a newer manual save.
      while (savePromise) await savePromise;
      if (disposed || !pagesState.selectedPage?._id) return false;
      cancelAutoSave();
      ...
      const pending = (async () => {
        try {
          ...
          if (disposed || String(pagesState.selectedPageId) !== String(pageId))
            return false;
          savedVersion = version;
          pagesState.lastSavedAt =
            data.page.updatedAt || new Date().toISOString();
          // Editing remains available during autosave; do not replace newer input.
          if (editVersion !== version) {
            setAutoSaveState("pending", "Unsaved changes");
            return false;
          }
          setAutoSaveState("saved", "Saved");
          pagesState.pages = pagesState.pages.map((page) =>
            String(page._id) === String(pageId) ? data.page : page,
          );
          if (!auto) {
            setPagesState({
              selectedPage: data.page,
          ...
          if (
            ...
            setAutoSaveState("error", error.message || "Could not save page");
            if (!auto)
              showPagesActionToast(
    ...
    async function publishPageAndReturnToLibrary({ featureOnHome } = {}) {
      ...
      if (!(await saveSelectedPage({ toast: false }))) return;
      try {
        ...
        showPagesActionToast(
          error.message || "Could not publish page",
          "error",
        );
      }
    }
    ...
    function moveBlockToPosition(index, layout) {
      ...
      if (!canPlaceBuilderBlock(nextLayout, { ignoreIndex: index }))
        return false;
    ...
    async function uploadColumnImage(index, columnIndex, file) {
      const data = await uploadImageToCdn(
        file,
      ...
      updateBlockColumn(index, columnIndex, {
        ...getUploadMediaUpdate(data),
        crop: getCrop(
          pagesState.selectedPage?.blocks?.[index]?.columns?.[columnIndex]
            ?.crop,
        ),
      });
    }
    ...
    pagesAdminContent.addEventListener(
      "click",
      (event) => {
    ...
    const ready = (async () => {
      ...
      if (permissions.canManagePages !== true) {
        showPagesStatus("Access denied", "error");
        return;
      }
      pagesState.canManageNavigation = permissions.canManageNavigation === true;
      pagesState.canFeaturePagesOnHome =
        permissions.canFeaturePagesOnHome === true;
      try {
        await loadPages();
      } catch (error) {
        if (!disposed) {
    ...
    return {
      ready,
      hasUnsavedChanges,
      canNavigate: () => !busy && gestureCleanups.size === 0,
      dispose() {
        disposed = true;
        lifecycle.abort();
        cancelAutoSave();

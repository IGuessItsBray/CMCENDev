...
const {
  ...
  getImageUrl,
  getPageContent,
} = require('./lib/bilingual-import');

const args = parseArgs();
const inputPath = resolvePath(
  args.input,
  path.join(__dirname, 'output', 'production-bilingual-source-inventory.json'),
);
const outputDir = resolvePath(args.output, path.join(__dirname, 'output'));
const manifestPath = resolvePath(
  args.manifest,
  path.join(outputDir, 'bilingual-import-dry-run-manifest.json'),
);
const checkpointPath = resolvePath(
  args.checkpoint,
  path.join(outputDir, 'bilingual-import-dry-run.checkpoint.json'),
);
...
if (args.apply) {
  throw new Error(
    'This importer is dry-run only. Review its manifest before enabling any write path.',
  );
}
  ...
  return {
    ...
    comments: comments
      .map((comment) => ({
        ...
        body: String(comment.content?.rendered || '')
          .replace(/<[^>]*>/gu, ' ')
          .replace(/\s+/gu, ' ')
          .trim(),
...
function summarize(results) {
  return {
    ...
    needsTranslation: results.filter((result) => result.needsTranslation)
      .length,
    fetchFailures: results.filter((result) => result.fetchError).length,
    identityReview: results.filter((result) =>
      result.issues.includes('identity-needs-review'),
    ).length,
    imageIssues: results.filter((result) => result.imageIssues?.length).length,
    messageIssues: results.filter((result) => result.messageIssues?.length)
      .length,
    comments: results.reduce(
      (sum, result) => sum + (result.comments?.length || 0),
      0,
    ),
    commentIssues: results.filter((result) => result.commentIssues?.length)
      .length,
  };
}
...
async function main() {
  ...
  if (!inventory.complete) {
    throw new Error(
      'The source inventory is incomplete. Resume it before preparing an import.',
    );
  }
  ...
  if (args.resume) {
    ...
    if (
      checkpoint.sourceInventory !== inputPath ||
      checkpoint.candidateCount !== candidates.length
    ) {
      throw new Error(
        'The import checkpoint does not match this inventory or candidate limit. Start a new checkpoint.',
      );
    }
    results = checkpoint.results || [];
    if (results.length > candidates.length) {
      throw new Error(
        'The import checkpoint has more results than this run allows. Start a new checkpoint.',
      );
    }
    console.log(
      `Resuming from ${results.length}/${candidates.length} prepared candidates.`,
    );
  }
  ...
  for (const [index, candidate] of candidates.entries()) {
    ...
    try {
      ...
      result.messages = Object.fromEntries(
        result.sources.map((source) => [source.language, source.message]),
      );
      result.images = Object.fromEntries(
        result.sources.map((source) => [source.language, source.imageUrl]),
      );
      ...
      result.wordpressPostIds = Object.fromEntries(
        result.sources.map((source, sourceIndex) => [
          source.language,
          commentResults[sourceIndex].postId,
        ]),
      );
      result.comments = [
        ...new Map(
          commentResults
            .flatMap((commentResult) => commentResult.comments)
            .map((comment) => [comment.wordpressCommentId, comment]),
        ).values(),
      ];
      result.commentIssues = commentResults
        .filter((commentResult) => commentResult.issue)
        .map((commentResult) => commentResult.issue);
      ...
      result.issues = [
        ...result.issues,
        ...result.imageIssues,
        ...result.messageIssues,
        ...result.commentIssues,
      ];
    } catch (error) {
    ...
    console.log(
      `[${index + 1}/${candidates.length}] Prepared ${candidate.contentType} candidate`,
    );
  }

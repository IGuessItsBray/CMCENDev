...
function assertPublicMediaBaseUrl(publicBaseUrl) {
  const internalEndpoint = String(process.env.MINIO_ENDPOINT || '').replace(/\/+$/u, '');
  const publicEndpoint = String(process.env.MINIO_PUBLIC_ENDPOINT || '').replace(/\/+$/u, '');
  const hasCdnBase = Boolean(process.env.CDN_PUBLIC_BASE_URL || process.env.CDN_BASE_URL);

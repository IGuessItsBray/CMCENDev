...
function getMinioPublicBaseUrl() {
  ...
  const bucketName = String(process.env.MINIO_BUCKET_NAME || '').replace(
    /^\/+|\/+$/gu,
    '',
  );

  return endpoint && bucketName ? `${endpoint}/${bucketName}` : endpoint;
}
...
function buildPublicMediaUrl(key) {
  ...
  return cleanKey ? `${getCdnBaseUrl()}/${encodeObjectKey(cleanKey)}` : '';
}
...
function getMediaKeyFromValue(value) {
  ...
  if (!/^https?:\/\//iu.test(rawValue)) {
    for (const knownBaseUrl of getKnownCdnBaseUrls()) {
      ...
      if (
        rawValue === knownBaseUrl ||
        rawValue.startsWith(`${knownBaseUrl}/`)
      ) {
  ...
  try {
    ...
    for (const knownBaseUrl of getKnownCdnBaseUrls()) {
      if (knownBaseUrl.startsWith('/')) {
        if (
          url.pathname === knownBaseUrl ||
          url.pathname.startsWith(`${knownBaseUrl}/`)
        ) {

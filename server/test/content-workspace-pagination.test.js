...
test('provides a paged Load more control for the content workspace list', () => {
  ...
  assert.match(workspaceScript, /query\.set\("cursor", contentWorkspaceState\.nextCursor\);/u);
  assert.match(
    workspaceScript,
...
test('localizes content workspace loading controls in English and French', () => {
  for (const language of ['en', 'fr']) {
    assert.equal(typeof translations[language].content_workspace_load_more, 'string');
    assert.equal(typeof translations[language].content_workspace_loading_more, 'string');
    assert.equal(typeof translations[language].content_workspace_load_history, 'string');
    assert.equal(typeof translations[language].content_workspace_retry_history, 'string');
  }
});

...
test('derives the documented French Last Post URL from its English source', () => {
  assert.equal(
    deriveFrenchLastPostUrl('https://cmcen-rcmce.ca/lp/last-post-martin-lavigne/'),
...
test('keeps only absolute content links when extracting an archive', () => {
  ...
  ].join(''), 'https://cmcen-rcmce.ca/last-post-years-archive/');
...
test('pairs documented Last Post translations without subject guessing', () => {
  const result = pairEntries([
    {
      ...
      contentType: 'last-post', language: 'en', title: 'Last Post - Martin Lavigne',
      subject: 'Last Post - Martin Lavigne', alternateUrls: {}
    },
    {
      url: 'https://cmcen-rcmce.ca/fr/lp/derniers-appels-martin-lavigne/',
      contentType: 'last-post', language: 'fr', title: 'Derniers appels - Martin Lavigne',
      subject: 'Derniers appels - Martin Lavigne', alternateUrls: {}
    }
  ]);
  ...
  assert.equal(normalizeSubject('Derniers appels - Martin Lavigne'), 'martin lavigne');
});
...
test('reads an authoritative French translation from the live-site WPML switcher', () => {
  ...
  assert.equal(isContentDetailUrl('https://cmcen-rcmce.ca/retirements/'), false);
  assert.equal(
    isContentDetailUrl('https://cmcen-rcmce.ca/retirement-warrant-officer-jason-st-pierre/'),
    true
  );
});
